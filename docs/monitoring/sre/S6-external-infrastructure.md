...
Unlike other SRE dashboards, S6 uses **rows** rather than tabs — each row is a distinct external system with its own data source. No shared "Health at a Glance" tab because these systems have no common metrics.
