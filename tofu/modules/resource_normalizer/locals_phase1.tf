    ...
    for cluster_key, cluster in var.resources : cluster_key => merge(
      ...
      # ── NORMALIZATION ─────────────────────────────────────────────────────
      {
        ...
        lxc_config = try(cluster.lxc_config, null) != null ? {
          ...
          template_datastore_id     = try(cluster.lxc_config.template_datastore_id, null)
          template_url              = try(cluster.lxc_config.template_url, null)
          template_checksum         = try(cluster.lxc_config.template_checksum, null)
          template_checksum_algorithm = try(cluster.lxc_config.template_checksum_algorithm, null)
        ...
        nodes = {
          for node_key, node in try(cluster.nodes, {}) : node_key => {
            ...
            lxc_config = try(node.lxc_config, null) != null ? {
              ...
              template_datastore_id     = try(node.lxc_config.template_datastore_id, null)
              template_url              = try(node.lxc_config.template_url, null)
              template_checksum         = try(node.lxc_config.template_checksum, null)
              template_checksum_algorithm = try(node.lxc_config.template_checksum_algorithm, null)
